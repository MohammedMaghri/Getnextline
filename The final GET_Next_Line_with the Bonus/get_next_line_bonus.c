...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/11/23 11:14:57 by mmaghri           #+#    #+#             */
/*   Updated: 2023/11/23 12:42:23 by mmaghri          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	static char	*copystring[OPEN_MAX];
	char		*line;
	char		*string;
	...
	if (!string)
		return (NULL);
	string[0] = '\0';
	copystring[fd] = buffercopy(fd, string, copystring[fd]);
	line = newlinefinder(copystring[fd]);
	if (!copystring[fd])
		return (NULL);
	copystring[fd] = freethfinal(copystring[fd]);
	return (line);
}
